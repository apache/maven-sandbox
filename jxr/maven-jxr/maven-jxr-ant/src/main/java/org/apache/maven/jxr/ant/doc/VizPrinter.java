...
public class VizPrinter
{
     ...
     * @param attrstmt attribute statement
     */
    protected void addAttributeStatement( Hashtable table, VizAttrStmt attrstmt )
    {
        VizASType type = attrstmt.getType();
        VizAttrStmt oldAttrstmt = (VizAttrStmt) table.get( type );
        if ( oldAttrstmt == null )
        {
    ...
    private void filterReferences()
    {
        if ( !"".equals( targetFrom ) )
        {
            ...
            VizTarget from = ( (VizProject) projects.elementAt( 0 ) ).getTarget( targetFrom );
            if ( from != null )
        ...
        {
            Vector targets = new Vector();
            VizTarget to = ( (VizProject) projects.elementAt( 0 ) ).getTarget( targetTo );
            if ( to != null )
    ...
    private void filterReferences( Vector targets )
    {
        ...
        while ( enumList.hasMoreElements() )
        {
            Enumeration targetEnum = ( (VizProject) enumList.nextElement() ).getOrderedTargets().elements();
    ...
    private void addReferredTargets( VizTarget target, Vector set, boolean backward )
    {
    ...
    {
        if ( isSubgraph && printAttrIfExists( subgraphAttrMap, VizASType.NODE_DEFAULT ) )
        {
            return;
        }
    ...
    public void print()
    {
        ...
        while ( enumList.hasMoreElements() )
        {
            printProject( (VizProject) enumList.nextElement(), clusterRefs, subgraphNum );
            subgraphNum++;
        }
    ...
    private void printProject( VizProject project, Vector clusterRefs, int subgraphNum )
    {
        if ( 0 < subgraphNum )
        {
            println( "<subgraph " + ( noCluster ? "" : "numcluster=\"" ) + subgraphNum + "\"" );
            indentLevel++;
            println( " label=" + getQuotedId( project.getDir() + " " + project.getFile() ) + " " );
        ...
        while ( targetEnum.hasMoreElements() )
        {
            printTarget( (VizTarget) targetEnum.nextElement(), project, clusterRefs, ( 0 < subgraphNum ) );
        }
        if ( 0 < subgraphNum )
        {
            indentLevel--;
    ...
    private void printTarget( VizTarget target, VizProject project, Vector clusterRefs, boolean isSubgraph )
    {
        ...
        {
            label = escapeId( project.getDir() + " " + project.getFile() ) + "\\n" + escapeId( label );
